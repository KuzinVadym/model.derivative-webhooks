            ...
            if (!alreadyTranslated) {
              ...
              request({
                ...
                objects.uploadObject(ossBucketKey, ossObjectName, filestream.length, filestream, {}, null, tokenInternal).then(function (response) {
                  ...
                  const jobBody = translateData(ossUrn);
                  jobBody.misc = { workflow: workflow };
                  request({
...
router.post('/integration/getMetaData', jsonParser, function (req, res) {
  var ossUrn = req.body.urn;

  var tokenSession = new token(req.session);
  tokenSession.getTokenInternal(function (tokenInternal) {
    var derivative = new ForgeSDK.DerivativesApi();
    derivative.getMetadata(ossUrn, {}, null, tokenInternal).then(function (response) {
      var metaData = response.body;
      res.status(200).json({
        extractCompleted: true,
        message: 'Getting Metadata completed',
        metaData: metaData
      });
    }).catch(function (e) {
      res.status(500).json({error: 'Can not get Meta Data, check urn or other GET params'});
    });
  });
});
